...
* Unprivileged user `notiv` (uid=1000, configurable, see options) in group `users` (gid=100) with ownership over `/home/notiv` and `/opt/conda`
...
```
docker run -d -p 8888:8888 notiv/sparkondocker
```
...
1. Configure each slave with [the `--no-switch_user` flag](https://open.mesosphere.com/reference/mesos-slave/) or create the `notiv` user on every slave node.
...
You can use this same approach to sidestep the `start-notebook.sh` script and run another command entirely. But be aware that this script does the final `su` to the `notiv` user before running the notebook server, after doing what is necessary for the `NB_USER` and `GRANT_SUDO` features documented below.
...
* `-e USE_HTTPS=yes` - Configures Jupyter Notebook to accept encrypted HTTPS connections. If a `pem` file containing a SSL certificate and key is not found in `/home/notiv/.ipython/profile_default/security/notebook.pem`, the container will generate a self-signed certificate for you.
* `-e NB_UID=1000` - Specify the uid of the `notiv` user. Useful to mount host volumes with specific file ownership.
